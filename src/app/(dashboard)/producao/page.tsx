...
import { ChevronRight, Wrench, Play, CheckCircle2, AlertCircle, Clock, Percent, Cpu, RefreshCw, Loader2 } from 'lucide-react';
import Link from 'next/link';
import { getOrdensProducao } from '@/actions/producao-actions';
...
export default function ProducaoPage() {
  ...
  async function loadData() {
    setIsLoading(true);
    try {
      const data = await getOrdensProducao();
      setOrders(data || []);
    } catch (error) {
      console.error(error);
    } finally {
  ...
  const getStatusLabel = (st: string) => {
    switch (st) {
      case 'aguardando': return 'Aguardando Insumo';
      case 'corte': return 'Corte Inicial';
      case 'borda': return 'Colagem de Borda';
      case 'furacao': return 'Furação CNC';
  ...
  return (
    <div className="p-8 bg-[#0F1115] min-h-screen text-slate-200 font-sans">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
        ...
        <button onClick={loadData} className="flex items-center px-4 py-2 bg-slate-800 hover:bg-slate-700 text-white rounded-lg transition-all text-sm font-medium border border-slate-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <div className="bg-[#1A1D24] border border-slate-800 p-6 rounded-xl shadow-sm">
          <div className="flex justify-between items-start mb-4">
            <h3 className="text-slate-400 font-medium text-sm">OPs Cadastradas</h3>
            <div className="p-2 bg-slate-800/50 rounded-md"><Percent className="w-5 h-5 text-blue-400" /></div>
          </div>
          <h2 className="text-3xl font-bold text-white tracking-tight">{orders.length}</h2>
          <span className="text-xs text-emerald-400 mt-2 block">Ordens de produção no Neon</span>
        </div>
        <div className="bg-[#1A1D24] border border-slate-800 p-6 rounded-xl shadow-sm">
          <div className="flex justify-between items-start mb-4">
            <h3 className="text-slate-400 font-medium text-sm">Maquinário Ativo</h3>
            <div className="p-2 bg-slate-800/50 rounded-md"><Cpu className="w-5 h-5 text-purple-400" /></div>
          </div>
          <h2 className="text-3xl font-bold text-white tracking-tight">{maquinas.filter(m => m.status === 'Operando').length} <span className="text-sm font-medium text-slate-500">de {maquinas.length}</span></h2>
          <span className="text-xs text-orange-400 mt-2 block">Máquinas em operação agora</span>
        </div>
        <div className="bg-[#1A1D24] border border-slate-800 p-6 rounded-xl shadow-sm">
          <div className="flex justify-between items-start mb-4">
            <h3 className="text-slate-400 font-medium text-sm">OEE Médio Fabril</h3>
            <div className="p-2 bg-slate-800/50 rounded-md"><Wrench className="w-5 h-5 text-orange-400" /></div>
          </div>
          <h2 className="text-3xl font-bold text-white tracking-tight">{Math.round(maquinas.reduce((a, b) => a + b.oee, 0) / maquinas.length)}%</h2>
          <span className="text-xs text-emerald-400 mt-2 block">Eficiência geral do equipamento</span>
        </div>
        <div className="bg-[#1A1D24] border border-slate-800 p-6 rounded-xl shadow-sm">
          <div className="flex justify-between items-start mb-4">
            <h3 className="text-slate-400 font-medium text-sm">Atraso de Cronograma</h3>
            <div className="p-2 bg-slate-800/50 rounded-md"><Clock className="w-5 h-5 text-emerald-400" /></div>
          </div>
      ...
      <div className="bg-[#1A1D24] border border-slate-800 rounded-xl overflow-hidden shadow-sm">
        <div className="flex border-b border-slate-800 px-6 py-4">
          <div className="flex space-x-6">
            <button onClick={() => setActiveTab('ops')} className={`pb-2 text-sm font-semibold transition-all relative ${activeTab === 'ops' ? 'text-orange-500' : 'text-slate-400 hover:text-white'}`}>
              Ordens de Produção (OP)
              {activeTab === 'ops' && <div className="absolute bottom-[-17px] left-0 right-0 h-0.5 bg-orange-500 rounded-full" />}
            </button>
            <button onClick={() => setActiveTab('maquinas')} className={`pb-2 text-sm font-semibold transition-all relative ${activeTab === 'maquinas' ? 'text-orange-500' : 'text-slate-400 hover:text-white'}`}>
          ...
          {activeTab === 'ops' ? (
            isLoading ? (
              ...
              <div className="text-center py-12 text-slate-500">
                ...
                <p className="text-lg font-medium text-slate-400">Nenhuma ordem de produção</p>
                <p className="text-sm mt-1">Crie projetos e vincule-os a ordens de produção para gerenciar o chão de fábrica.</p>
              </div>
            ) : (
                  ...
                  <tbody className="divide-y divide-slate-800/40">
                    {orders.map((order) => (
                      <tr key={order.id} className="hover:bg-slate-800/20 transition-all duration-150">
                        <td className="py-4 font-medium text-slate-200">{order.projetoNome}</td>
                        <td className="py-4 text-slate-400">{order.clienteNome}</td>
                        <td className="py-4 text-slate-400 font-mono text-xs">
                          {order.dataInicio ? new Date(order.dataInicio).toLocaleDateString('pt-BR') : '---'}
                        </td>
                        <td className="py-4 text-right">
                          <span className={`px-2.5 py-1 rounded-full text-xs font-semibold border ${
                            order.status === 'expedicao' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' :
                            order.status === 'corte' || order.status === 'furacao' ? 'bg-blue-500/10 text-blue-400 border-blue-500/20' :
                            order.status === 'aguardando' ? 'bg-slate-800 text-slate-400 border-slate-700' :
              ...
              {maquinas.map((maquina) => (
                <div key={maquina.nome} className="bg-[#0F1115] border border-slate-800 p-6 rounded-xl relative overflow-hidden">
                  ...
                  <div className="space-y-3 font-mono text-xs text-slate-400 border-t border-slate-800/80 pt-4">
                    <div className="flex justify-between"><span>Temperatura</span><span className="text-slate-200">{maquina.temperatura}</span></div>
                    <div className="flex justify-between"><span>Rendimento Atual</span><span className="text-orange-400">{maquina.pecasHora} peças / hora</span></div>
                  </div>
